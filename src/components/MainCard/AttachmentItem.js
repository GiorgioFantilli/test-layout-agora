    ...
    <div className="attachment-item">
      ...
      <div className="attachment-header">
        ...
        <div className="attachment-actions">
          <button className="link-button mr-[-0.5rem]" onClick={handlePreview} title="Visualizza">
            <i className="fas fa-eye"></i>
          </button>
          <button className="link-button" onClick={handleDownload} title="Scarica">
      ...
      {/* ── Sintesi AI (hero) — skeleton durante elaborazione, contenuto quando pronto ── */}
      {(jobStatus === 'QUEUED' || jobStatus === 'IN_PROGRESS') && (
        <div className="attachment-synthesis-section">
          <div className="synthesis-label" style={{ marginBottom: '0.5rem' }}>
            <i className="fa-solid fa-wand-magic-sparkles" style={{ opacity: 0.5 }}></i>
            <span style={{ opacity: 0.6 }}>Sintesi AI</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
            <div className="timeline-skeleton-pulse" style={{ height: '0.72rem', borderRadius: '4px', width: '88%' }} />
            <div className="timeline-skeleton-pulse" style={{ height: '0.72rem', borderRadius: '4px', width: '65%' }} />
            <div className="timeline-skeleton-pulse" style={{ height: '0.72rem', borderRadius: '4px', width: '42%' }} />
          </div>
        </div>
      )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.3rem' }}>
                  ...
                  {visibleEntities.shown.length > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', flexWrap: 'wrap' }}>
                      {visibleEntities.shown.map((entity, i) => (
                        <span
                          key={i}
                          style={{
                            fontSize: '0.65rem', fontWeight: 600,
      ...
      {/* Legacy plain-text (solo se non c'è analisi strutturata) */}
      {!documentAnalysis && analysisResult && (
        <div className="attachment-synthesis-section">
          ...
          <p className="synthesis-text">{analysisResult}</p>
        </div>
      )}

      {/* ── Testo estratto (secondario, collapsed) ── */}
