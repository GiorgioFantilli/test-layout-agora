...
// --- Tipi di file ---
// Per separare i dati dalla logica, definiamo solo i tipi MIME qui
const FILE_TYPES = {
    pdf: 'application/pdf',
    jpg: 'image/jpeg',
    png: 'image/png',
    ...
    eml: 'message/rfc822',
    xml: 'application/xml',
    dat: 'application/octet-stream', // NUOVO (Richiesta 9)
    log: 'text/plain', // NUOVO (Richiesta 9)
};


// --- Demo Data (Aggiornata) ---
const emailData = {
    // --- NON LETTE (Unread) ---
    'unread1': { 
        sender: 'Mario Rossi', 
...
const initialState = {
    ...
    visuallyUnreadId: null, // MODIFICATO: Per la logica di lettura (Richiesta 4)
    isFullscreen: false,
...
function appReducer(state, action) {
    switch (action.type) {
        case 'SWITCH_VIEW':
            // MODIFICATO: Rimosso selectedEmailId: null (Richiesta 7)
            // MODIFICATO: Azzera visuallyUnreadId (Richiesta 4)
        ...
        case 'SELECT_EMAIL':
            ...
            // Se la *nuova* email selezionata è 'unread':
            // 1. Marcala 'read' *nei dati* (simula DB)
            // 2. Imposta 'visuallyUnreadId' per mantenerla in "Non Lette"
            if (newEmailsOnSelect[newSelectedId] && newEmailsOnSelect[newSelectedId].status === 'unread') {
                newEmailsOnSelect[newSelectedId] = { 
                    ...newEmailsOnSelect[newSelectedId], 
                    status: 'read',
                    readDate: new Date().toISOString() // Simula salvataggio data lettura
                };
